...
# from src.models.ip_adapter.attention_processor import SparseCausalAttentionProcessor

if hasattr(F, "scaled_dot_product_attention"):
    from .attention_processor import IPAttnProcessor2_0 as IPAttnProcessor, AttnProcessor2_0 as AttnProcessor, CNAttnProcessor2_0 as CNAttnProcessor, SparseCausalAttentionProcessor
else:
    from .attention_processor import IPAttnProcessor, AttnProcessor, SparseCausalAttentionProcessor, CNAttnProcessor
...
class IPAdapter(torch.nn.Module):
    def __init__(self, unet, model_path, subfolder="ip_adapter"):
        super().__init__()
        
        main_path = os.path.join(model_path, subfolder, "models")
        image_encoder_path = os.path.join(main_path, "image_encoder")
        
        self.ip_adapter_model = torch.load(os.path.join(main_path, "ip-adapter_sd15.bin"), map_location="cpu", weights_only=True)
        
        # detect features
        self.is_plus = "latents" in self.ip_adapter_model["image_proj"]
        self.output_cross_attention_dim = self.ip_adapter_model["ip_adapter"]["1.to_k_ip.weight"].shape[1]
        ...
        # set image encoder and load clip image encoder weights
        self.image_encoder = CLIPVisionModelWithProjection.from_pretrained(image_encoder_path)#.to(self.device, dtype=self.dtype)
        self.clip_image_processor = CLIPImageProcessor(resample=Image.Resampling.LANCZOS)
        
        # set IPAdapter
        self.set_ip_adapter(unet)
        self.image_proj_model = self.init_proj() if not self.is_plus else self.init_proj_plus()
        
        # load the weights of the image_projection model 
        self.image_proj_model.load_state_dict(self.ip_adapter_model["image_proj"])
        
        # get the current state_dict of the ip_layers
        ip_layers = torch.nn.ModuleList(unet.attn_processors.values())

        # map the layers from old model to new one 
        new_state_dict = {}
        for i, (name_ip_curr, name_ip_ori) in enumerate(zip(ip_layers.state_dict().keys(), self.ip_adapter_model["ip_adapter"].keys())):
            new_state_dict[name_ip_curr] = self.ip_adapter_model["ip_adapter"][name_ip_ori]
        
        ip_layers.load_state_dict(new_state_dict)
        
        self.set_tokens(unet, num_tokens=self.num_tokens)
    ...
    def set_ip_adapter(self, unet):
        ...
        for name in unet.attn_processors.keys():
            cross_attention_dim = None if name.endswith("attn1.processor") or name.endswith("attn_temp.processor") else unet.config.cross_attention_dim
            is_sparse_causal_attn = True if name.endswith("attn1.processor") else False
            ...
            if cross_attention_dim is None and not is_sparse_causal_attn:
                attn_procs[name] = AttnProcessor()
            elif cross_attention_dim is None and is_sparse_causal_attn:
                attn_procs[name] = SparseCausalAttentionProcessor()
            else:
    ...
    def set_tokens(self, unet, num_tokens):
